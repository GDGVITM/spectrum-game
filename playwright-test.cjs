...
(async () => {
  ...
  await page.waitForFunction(() => !document.body.innerText.includes("Loading Game Assets..."), { timeout: 30000 });
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (txt.includes("attack") || txt.includes("enter") || txt.includes("spectrum")) {
